...
test('addCollection', async ({ page, context }) => {
  ...
  await page.waitForTimeout(1 * 1000);
  await page.locator("//input[@placeholder='e.g Summer collection, Under $100, Staff picks']").fill('Mobile phone'); // hoặc .type('text', { delay : 100, })
  await page.waitForTimeout(1 * 1000);
  ...
  await page.click("//div[@class='s-modal-body']//div[2]//div[1]//label[1]//span[1]");
  ...
  await collectionStorefrontPage.waitForTimeout(5*1000);
